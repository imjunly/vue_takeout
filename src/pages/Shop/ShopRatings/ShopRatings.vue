    ...
    computed: {
      ...
      filterRatings() {
        ...
        return ratings.filter(rating => {
          ...
          /**
           * 条件1
           * selectType: 0/1/2 ：评论类型
           * rateType: 0/1, 但是已有数据里面只有0和1, 如果我们点击满意时，selectType就是2， 否则我们就看rateType
           * selectType === 2 || selectType === rateType
           * 如果选择类型为2的评论，则返回所有类型为2的评论，否则返回rateType的评论，注意到rateType只有0/1，因此只要第一个条件成立，后面的条件不可能成立，因此可以返回所有2的评论
           * 如果选择类型为1的评论，则第一个条件不成立。
           *
           *
           * 条件2
           * text: 有值/没值
           ...
           *
           * 如果选择了仅显示带文字的，则返回所有评论中，长度大于0的评论, (逻辑值：false || 长度大于0的} ==> 所有长度大于0的评论
           * 如果没有选择带评论的，那么所有都显示 {true || true} ==> 所有评论
